...
class Player(BasePlayer):
    ...
    selected_for_payment = models.BooleanField(initial=False)
...
def creating_session(subsession: Subsession):
    for p in subsession.get_players():
        if subsession.round_number == 1:
            ...
            # select payment round
            p.participant.vars['payment_round'] = random.randint(1, C.NUM_ROUNDS)
            p.participant.vars['selected_for_payment'] = random.random() <= C.PAYMENT_PROBABILITY
...
class Decision(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        if player.round_number == player.participant.vars['payment_round'] and player.participant.vars.get('selected_for_payment', False):
            pay_for = random.choice(['heads', 'tails'])
            player.participant.vars['paid_for'] = pay_for
            if pay_for == 'heads':
                player.payoff = heads
            else:
                player.payoff = tails
